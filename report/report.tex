...
Forest, XGBoost, MLP, LightGBM, and a hyperparameter-tuned LightGBM — against
a naive lag-1h baseline, and find that the tuned LightGBM achieves a test RMSE
of \textbf{1.501} and MAE of \textbf{0.759} departures per hour, a 25.6\%
RMSE improvement over the naive baseline. Analysis reveals that short-term
lag features dominate predictive performance, PM rush hour is the hardest
time to predict, high-error stations concentrate in high-demand Midtown
Manhattan, and a sample-complexity study shows that all models saturate at
roughly 10\% of the training data — indicating that feature expressiveness,
not data volume, is the main bottleneck on this task.
\end{abstract}
