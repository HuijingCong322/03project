...
% ── Abstract ──────────────────────────────────────────────────────────────────
\begin{abstract}
NYC Citi Bike, like other bike-sharing systems, often faces the problem of imbalanced demand across different stations and at different times.  In this project, we model this as a regression task. To predict bike departures at each station in a given hour, we use two years of Citi Bike trip records, hourly weather data, and calendar data. We train six models, namely Ridge Regression, Random Forest, XGBoost, MLP, LightGBM, and hyperparameter-tuned LightGBM, and compare them with a lag-1h baseline. To better predict demand, we experimentally include the following elements in our project: short-term lag features, systematic hyperparameter tuning, and training with a multi-year trip dataset. The results show that the hyperparameter-tuned LightGBM has the best performance, with an RMSE of \textbf{1.501} and an MAE of \textbf{0.759}. It outperforms the simple lag-1h baseline by 25.6\% in RMSE and Ridge Regression by 12.7\% in RMSE. The results also suggest that better features, such as short-term lag features in this case, rather than sheer data volume, play a more important role in improving the accuracy of demand forecasting.
\end{abstract}

% ── 1. Introduction ───────────────────────────────────────────────────────────
\section{Introduction}

In recent years, bike-sharing systems have become an increasingly important mode of transportation for urban populations. Citi Bike is the largest bike-sharing system in New York City, with bike stations across Manhattan, Brooklyn, Queens, and the Bronx. However, a major challenge for Citi Bike is demand imbalance across different areas and at different times. This requires the manual operation of moving bikes between stations, which is both time-consuming and costly. Better demand prediction can alleviate this problem by allowing operators to redistribute bikes in advance. 

To address this challenge, we model it as a regression task. Using historical trip records together with weather and calendar information, our models predict how many bikes will depart from a given station during a given hour. We train and compare six models: Ridge Regression, Random Forest, XGBoost, MLP, LightGBM, and hyperparameter-tuned LightGBM. Through this comparison, we aim to enhance prediction accuracy and better understand demand patterns in the NYC bike-sharing system.
...
and MAE of 1.62. We note, however, that direct numerical comparison across studies should be interpreted with caution: train/test splits, station coverage, and
aggregation periods differ across works, so improvements may partly reflect
data setup rather than model capability alone.
...
We use three data sources in our work. The first data source is the \textbf{Citi Bike trip history data} for the full years 2023 and 2024. The trip records include start time and date, stop time and date, start and end station IDs, and rider type, either member or casual. We process the raw trip records and aggregate them into hourly departure counts for each station. The hourly departure count for each station is our prediction target. 

The weather data are collected by querying the \textbf{Open-Meteo Archive API} for hourly weather observations from Central Park. We use temperature (fahrenheit), precipitation (inches), snowfall (inches), and snow depth (inches). Although a single weather station may not fully represent the local weather conditions across all stations, it can capture the major effects of weather on bike demand. 

We extract all New York State federal holidays from January 2023 to December 2024 using the Python \texttt{holidays} package. The holidays are encoded as a binary feature, which takes a value of 1 for all hours falling on that day.

\subsection{Preprocessing and Feature Engineering}

We aggregate raw trip records into a station by hour demand grid. To make sure lag features can be computed correctly, we assign zero values to hours with no departures. The first 24 hours of each station's history are dropped because lag values are
unavailable. The full grid covers 2,453 active stations and 17,520 hourly time slots, producing approximately 43 million station-hour rows.
...
\begin{itemize}
  ...
  \item \textbf{Lag features:} departure count at the same station 1, 2, and 24
        hours prior. These features help the model use recent data to predict current demand.
\end{itemize}
...
The time-based split, instead of a random split, better reflects real-world conditions, where a model is trained on historical data and evaluated on future time periods.
...
\textbf{Random Forest} \citep{breiman2001random} is a nonlinear ensemble of
...
validation and test sets, alongside a naive baseline that simply predicts
the previous hour's demand at the same station ($\hat{y} = \text{lag\_1h}$).

\begin{table}[H]
\centering
\caption{Model performance on validation (Oct--Nov 2024) and test (Dec 2024) sets.
         RF and MLP are trained on a 5M-row random subsample due to memory
         constraints; all other models use the full 38M-row training set.
         Naive baseline simply outputs the previous hour's departures (lag\_1h).}
\label{tab:results}
\begin{tabular}{lcccc}
\toprule
Model & Val RMSE & Val MAE & Test RMSE & Test MAE \\
\midrule
Naive (lag\_1h)      & 3.119 & 1.479 & 2.017 & 0.923 \\
\midrule
...
All trained models clearly outperform the naive baseline. The tuned LightGBM
reduces test RMSE by 25.6\% (2.017 $\rightarrow$ 1.501) and test MAE by
17.8\% (0.923 $\rightarrow$ 0.759) compared to the naive predictor, and even
the linear Ridge model improves test RMSE by 14.7\%. This confirms that the
nonlinear models are learning meaningful patterns beyond simple temporal
persistence, and that the additional features (lag\_2h, lag\_24h, weather,
calendar, station coordinates) contribute real predictive value rather than
merely encoding lag\_1h in a different form.

Among the trained models, the tuned LightGBM achieves the best performance
on both splits. Compared to Ridge Regression, it reduces test RMSE by 12.7\%
and MAE by 9.7\%. The gap between LightGBM and the other nonlinear models
is relatively small — XGBoost comes close (test RMSE 1.536) — but
LightGBM's efficiency on the full 38M-row training set gives it an advantage
over Random Forest and MLP, which are trained on a 5M-row random subsample
due to memory constraints; this subsampling makes their comparison with
full-data models approximate rather than exact. Hyperparameter tuning provides only a marginal additional gain over the default
LightGBM (0.002 RMSE on the test set). This modest improvement likely reflects
that the model is already near the noise floor of the task: with 62\% of
test rows having zero departures and demand dominated by strong lag
autocorrelation, the remaining variance is largely stochastic and not
reducible by further tuning the search space.
...
\begin{figure}[H]
  ...
  \caption{Actual vs.\ predicted departures in three views. \textbf{Top-left:} hexbin density (log-scale) of all 1.85M
           predictions.
           \textbf{Top-right:} city-wide
           daily total departures. \textbf{Bottom:} hourly predictions for the highest-error station
           (W~21~St~\&~6~Ave, RMSE = 5.58).}
  \label{fig:avp}
\end{figure}

Figure~\ref{fig:avp} provides three views of model fit: overall prediction accuracy across the test set, city-wide demand over time, and single-station demand over time. \textbf{Top-left:} The
hexbin scatter shows that low bike demands are easier to predict, while the high demand periods has more variability. \textbf{Top-right:} The
plot shows that the model captures the general pattern of bike demands pretty well across December,
including the Christmas holiday dip (December 24--26). But the model predictions are not as sharp as the actual demand. \textbf{Bottom:} At the busiest and highest-error station
(W~21~St~\&~6~Ave, RMSE = 5.58), the model predictions follows the daily demand cycle
well but tend to underestimate the largest peaks.
...
To understand "how much data do different methods need to perform well", we trained
...
3.8M to 38.1M rows). The validation set is Oct and Nov 2024 data across all
...
As shown in Figure~\ref{fig:sample}, all nonlinear models saturate by 40\% of the training data, indicating additional data yield diminishing returns. Unexpectedly, Ridge Regression performs worse with more data. Two possible reasons are that the linear model is underfitting or there is a seasonal demand shift
between training (Jan 2023 to Sep 2024) and validation (Oct and Nov 2024). Overall, the model ranking stays the same across all
fractions:
LightGBM $>$ XGBoost $>$ Random Forest $>$ Ridge.

We conclude that for this task, 
feature quality and expressiveness is more important than data quantity. Future improvements
are more likely to come from better features.
...
calendar features alone. However, errors scale strongly with demand level:
high-demand stations in Midtown Manhattan show RMSE up to 5.58 departures per
hour, and the PM rush hour is the hardest period to predict (RMSE = 3.02).
These high-error cases represent precisely the operationally critical
scenarios where accurate forecasts matter most --- a limitation that future
work should address directly. Day of week has little impact overall, while
colder months with lower peak demand are easier to predict.
